// May Seter 312123037, Nir Hazan 316009489
import java.util.concurrent.*;

public class SemaphoreExample {
    static class SharedResource {
        private final Semaphore semaphore;

        public SharedResource(int permits) {
            //יש להשלים את החסר כאן
            this.semaphore = new Semaphore(permits,false);
        }

        public void accessResource() {
            try {
                //יש להשלים את החסר כאן // Acquire a permit
                this.semaphore.acquire(); // acquire a permit if there's an available one. if not, wait.
                System.out.println(Thread.currentThread().getName() + " is accessing the resource.");
                // Simulating some work
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                e.printStackTrace();
            } finally {
                // Release the permit // יש להשלים את החסר כאן
                this.semaphore.release(); // release and notifyAll();
            }
        }
    }

    static class Worker extends Thread {
        private final SharedResource sharedResource;

        public Worker(String name, SharedResource sharedResource) {
            super(name);
            this.sharedResource = sharedResource;
        }

        @Override
        public void run() {
            //יש להשלים את החסר כאן
            int num = 10;
            while(num > 0){
                this.sharedResource.accessResource();
                num--;
            }
        }
    }

    public static void main(String[] args) {
        SharedResource sharedResource = new SharedResource(2); //  permits available

        // Create multiple threads trying to access the resource
        Thread thread1 = new Worker("Thread 1", sharedResource);
        Thread thread2 = new Worker("Thread 2", sharedResource);
        Thread thread3 = new Worker("Thread 3", sharedResource);

        thread1.start();
        thread2.start();
        thread3.start();
    }
}
